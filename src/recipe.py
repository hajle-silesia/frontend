...
async def create_recipe():
    ...
    recipe_upload = HTML('input', upload_container.element,
                         {'id': "recipe_upload", 'type': "file", 'py_change': "upload_file()"})
    label_recipe_upload = HTML('label', upload_container.element,
                               {'id': "label_recipe_upload", 'html_for': "recipe_upload",
                                'inner_html': "Upload recipe &#129045;"})
    await show_last_file_content()
    pyodide.ffi.wrappers.add_event_listener(recipe_upload.element, "change", upload_file_and_show_its_content)
